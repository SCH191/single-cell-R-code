...
##数据基础分析
# 读取loom文件
velo <- read.loom.matrices("G:/SCG71.loom")
# 转换为seurat对象
...
nPC=1:20
velo <- FindNeighbors(velo, dims = nPC) %>% FindClusters() %>% RunUMAP(dims = nPC) %>% RunTSNE(dims = nPC)
...
##速率分析
velo <- RunVelocity(velo, deltaT = 1, kCells = 25, fit.quantile = 0.02, spliced.average = 0.2, unspliced.average = 0.05, ncores = 1)
